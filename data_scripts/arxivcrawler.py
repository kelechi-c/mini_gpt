...
out_text_file = f'arxiv_papers_{section}.txt'

def clean_text(text): # regex cleaning function
    regex_format = r"[^\w\s\d]"  # Leaves behind only text, digits, and whitespace characters
...
def merge_pdfs(file_list: list, out_text_file: str):
    ...
    try:
        for pdf in tqdm(file_list, colour="blue"):  # extract from pdf in queue
            ...
            text_corpus += (
                text_extract  # concat text and add space before the next one is added
            )
            text_corpus += "\n  "
        ...
        # success message
        rich.print(
            f"{len(pdf_list)} research paper PDFs extracted to single text file [bold green]{out_text_file}[/bold green]"
        )
        return text_corpus
